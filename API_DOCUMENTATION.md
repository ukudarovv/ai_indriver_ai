...
- `car_status` - Статус автомобиля: "good" (в хорошем состоянии) или "damaged" (поврежден)
- `car_status_text` - **Готовый текстовый отчет** с полным анализом автомобиля на русском языке
- `damage_count` - Количество найденных повреждений
- `cleanliness` - Уровень чистоты: "чистый", "слегка грязный", "грязный"
- `confidence` - Уверенность в оценке целостности (0-100%)
- `processing_time` - Время обработки в секундах

### Формат car_status_text
...
const analyzeCar = async (imageUri) => {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            showAnalysisText(data.car_status_text);
            
            // Показываем краткую информацию
            showCarStatus(data.car_status);
